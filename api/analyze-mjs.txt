...
- sodium is milligrams for the portion. Always estimate it. Only near-zero for plain
  unsalted produce, plain rice or water. Home cooking with seasoning, cheese, bread,
  sauces, cured meat and all restaurant food carry substantial sodium.
...
{"items":[{"name":"Fried chicken tenders","portion":"5 pieces","protein":38,"carbs":22,"fat":20,"kcal":420,"sodium":1100,"confidence":"high"}],"note":"one short sentence on assumptions"}`;
...
function notesBlock(notes) {
  const n = String(notes || "").trim().slice(0, 600);
  if (!n) return "";
  return `

HOW THIS USER COOKS AND EATS — standing notes they wrote themselves:
${n}

Apply these when estimating a home-cooked photo. They override your default assumptions
about cooking fat, portion size and preparation. Ignore them when the photo is clearly
from a restaurant, bar, cafeteria or takeout container.`;
}
      ...
      messages = [
        {
          ...
          content: [
            ...
            { type: "text", text: body.mode === "workout" ? WORKOUT_PROMPT : PHOTO_PROMPT + pantryBlock(body.pantry) + notesBlock(body.notes) + (body.hint ? `\n\nUser hint about this meal: ${body.hint}` : "") },
          ],
        },
      ];
    } else if (body.query) {
      messages = [{ role: "user", content: TEXT_PROMPT(String(body.query).slice(0, 300)) + notesBlock(body.notes) }];
    } else {
